...
This repo doubles as a Next.js demo app and as the source of the `@luvikung/three-next` package (built from [src/lib/three-next](src/lib/three-next)).

```bash
npm install @luvikung/three-next
```

If you'd rather not go through the npm registry, every release is also snapshotted onto a git branch that _is_ the built package, so it installs directly from GitHub:

```bash
npm install github:LuviKunG/three-next#npm/latest      # newest release
npm install github:LuviKunG/three-next#npm/v1.3.0       # pinned version
```

> `npm/*` branches are generated by `scripts/release-package.mjs` (via `npm run package:release:push`, or CI on a tagged release) — don't edit them directly; they're overwritten on every release.

`react` and `three` are peer dependencies — install them alongside the package if your project doesn't already have them.
